...
* **Bravo Chávez, Javier Alonso:** Responsable de la estrategia comercial, análisis económico de los escenarios y diseño de los indicadores de éxito (KPIs).
* **Castillo Braschi, Isabella María:** Responsable de la gestión del proyecto, coordinación del equipo y de estructurar la unión y cruce de las bases de datos en el clúster.
* **Corzo Quispe, Ernesto Rodrigo:** Responsable de la analítica predictiva, desarrollo del modelo de Inteligencia Artificial (Machine Learning) y evaluación de su precisión.
* **Godoy Castillo, Diego Alonso:** Responsable del control de calidad de los datos, corrección de valores nulos, baches lógicos y de la auditoría del uso de herramientas de IA.
* **Martell Lino, Giancarlos Arcadio:** Responsable del diseño y desarrollo del Dashboard final, asegurando que los datos procesados se visualicen de forma clara para el negocio.

---

## 2. Descripción del Problema de Negocio (Relevancia de la Fase 2)

**Empresa afectada:** Es una startup de movilidad urbana enfocada en transformar el transporte tecnológico en la ciudad de Nueva York.

**Relevancia Actualizada:** Mientras que en la Fase 1 el problema se limitó a la observación empírica de fluctuaciones de viajes, en la Fase 2 el desafío escaló a la optimización de la rentabilidad del activo. La asimetría en la distribución de la flota provoca una pérdida masiva de viajes y deserción de conductores en condiciones climáticas adversas y en zonas periféricas por falta de incentivos económicos automáticos. 

Para resolver esto, NeoCab requiere abandonar los esquemas de precios estáticos tradicionales e implementar un sistema distribuido de **fijación dinámica de precios (Surge Pricing) e inteligencia predictiva de despacho viales**, permitiendo mitigar el desabastecimiento crónico de unidades.

---

## 3. Descripción de los Datos Consolidados

Tras el proceso de ingeniería de datos en Databricks, las especificaciones de nuestra base de datos distribuida maestro son:

* **Fuente Principal:** Registros históricos de viajes de vehículos de alta densidad (High-Volume For-Hire Vehicle Trip Records - Uber/Lyft) en NYC. Volumen unificado: **45,931,107 registros** combinando verticalmente los meses de agosto, septiembre y octubre del 2021.
* **Fuentes de Enriquecimiento:** 
  1. Base de datos climatológica por hora (temperatura, precipitación, tipo de condiciones).
  2. Diccionario espacial oficial de zonas de taxis (mapeo de IDs de zonas a distritos o *Boroughs*).
  3. Calendario oficial de feriados en EEUU.
  4. Capa geoespacial de *Hotspots de Vida Nocturna* (zonas de alta concentración de ocio nocturno).
* **Variables Clave del Dataset Maestro (`df_estudio`):** `base_passenger_fare` (Target numérico), `trip_miles`, `trip_time`, `temp` (Casteada a Double), `precip` (Casteada a Double), `conditions`, `Borough`, `es_feriado`, y `is_nightlife_hotspot`.

---

## 4. Matriz de Evolución y Mejoras (PC1 vs PC2)

Para evidenciar la transformación del proyecto frente a la entrega pasada, se detalla la siguiente bitácora de auditoría de código:

| Componente | Estado en la Entrega Pasada (PC1) | Cambios y Modificaciones en la PC2 | Nuevas Adiciones (Exclusivo PC2) |
| :--- | :--- | :--- | :--- |
| **Infraestructura de Datos** | Carga aislada en Pandas/Spark de meses individuales (Agosto, Septiembre por separado). Celdas redundantes de verificación. | Se unificaron verticalmente los 45,931,107 de filas mediante `.unionByName()`. Se corrigieron baches lógicos de fechas convirtiendo cadenas a tipo fecha real con `.to_date()`. | **Persistencia Avanzada:** Implementación de `.cache()` estratégico sobre el dataset maestro para evitar colapsar los nodos del clúster durante consultas repetitivas. |
...
| **Nivel 2: Analítica Diagnóstica** | Enunciados de las 3 Hipótesis descritos únicamente en bloques de texto plano, sin respaldo empírico. | Se migró el análisis de texto a código real mediante agregaciones y agrupaciones masivas (`.groupBy()`) para contrastar las variables de transporte vs factores externos. | **Métrica de Velocidad:** Creación en tiempo de ejecución de la métrica de eficiencia operacional corporativa: **Velocidad Promedio en Millas por Hora (MPH)**. |
| **Nivel 3: Analítica Predictiva** | Inexistente en la entrega anterior (Fase exploratoria). | **Nueva Implementación:** Se diseñó el flujo predictivo desde cero al pasar a la fase de modelado estadístico avanzado en Big Data. | **Modelado Estadístico:** Construcción, entrenamiento y validación de un modelo de Regresión Lineal utilizando la librería `scikit-learn`, evaluado mediante R^2 y RMSE en test, incluyendo la extracción de coeficientes individuales. |
| **Nivel 4: Analítica Prescriptiva** | Inexistente en la entrega anterior (Fase exploratoria). | **Nueva Implementación:** Se desarrollaron los algoritmos de simulación económica en PySpark basados en los hallazgos del diagnóstico y el peso de las variables del modelo. | **Simulaciones de Impacto:** Creación de dos escenarios financieros matemáticos automatizados ("¿Qué pasaría si...?") y fijación de recomendaciones estratégicas amarradas a KPIs con metas numéricas medibles. |
...
El cálculo avanzado con `F.percentile_approx()` demostró que la **mediana** de las tarifas cobradas se ubica en **$14.20**, mientras que el **Percentil 75** alcanza los **$18.50**. Esto confirma una marcada asimetría positiva en la recaudación (una base masiva de viajes cortos micro-urbanos pero estables con picos extremos aislados de larga distancia). Esta estructura justifica la urgencia de abandonar la tarifa plana o lineal tradicional e implementar un motor de precios adaptativo para capturar el excedente del consumidor en los cuartiles superiores.

### Nivel 2: Analítica Diagnóstica (Evolución y Validación de Hipótesis)
Para esta entrega final, el equipo migró de preguntas exploratorias iniciales (Fase 1) a hipótesis analíticas refinadas y cuantitativas (Fase 2), evaluándolas mediante agregaciones masivas (`.groupBy`) sobre el clúster unificado:

#### Bloque de Investigación 1: Sensibilidad por Distrito y Factores Festivos
#### Evidencia Gráfica - Hipótesis 1
...
* **Resultado del Diagnóstico:** **VALIDADA.** Al agrupar por distrito y rangos térmicos, se demostró que cuando la temperatura desciende de los 10°C, la demanda en Queens y Brooklyn se dispara en un **14.2%** en comparación con días templados, mientras que Manhattan se mantiene estable por su densa red de metro. Además, al cruzar el flag de feriados, los cuadrantes clasificados como hotspots registraron un estiramiento en los cuartiles de precios, validando que la movilidad se concentra de forma masiva en zonas de ocio y consumo durante días no laborables.

#### Bloque de Investigación 2: Operación Eficiente vs. Reactiva bajo Lluvia
#### Evidencia Gráfica - Hipótesis 2
![Brecha de Precios: Lluvia vs Hotspots](Captura de pantalla 2026-06-22 a la(s) 6.56.27 p. m..png)
*Figura 2: Análisis de brecha de pricing (Pricing Gap Analysis) demostrando que las tarifas actuales bajo lluvia intensa no capturan el excedente del consumidor.*
...
* **Resultado del Diagnóstico:** **VALIDADA.** Mediante el cálculo en tiempo de ejecución de la métrica de velocidad promedio, se comprobó que bajo lluvia intensa la velocidad de los autos cae drásticamente de **11.4 MPH** a solo **6.1 MPH** (una reducción crítica del 46.5%). Al unirse el factor de día feriado, esta congestión vial severa alarga los viajes activos, atrapando a las unidades en ruta e impidiendo que regresen a los hotspots de alta demanda. Esto genera de manera predecible picos alarmantes en los tiempos de espera de los usuarios y un desabastecimiento generalizado en el sistema.

### Nivel 3: Analítica Predictiva (Modelo de Machine Learning)
#### Panel de Evaluación y Diagnóstico Estadístico del Modelo
![Evaluación Avanzada del Modelo de Regresión](Captura de pantalla 2026-06-22 a la(s) 6.56.04 p. m..png)
*Figura 4: Panel analítico cuádruple: (A) Valores reales vs. predichos, (B) Histograma de frecuencias de error (Residuos), (C) Importancia y peso neto de las variables explicativas y (D) Gráfico de homocedasticidad.*

> ### ⚠️ ANOMALÍA ESTADÍSTICA IDENTIFICADA — EFECTO COLINEAL DE "ES_FERIADO"
> ![Detalle de la anomalía en la Celda 98](Captura de pantalla 2026-06-22 a la(s) 6.56.48 p. m..png)
> *Figura 5: Captura de la consola del Notebook (Celda 98) que documenta el comportamiento del estimador multifactorial ante la multicolinealidad.*
Se entrenó un modelo de **Regresión Lineal** utilizando la librería estadística `scikit-learn` alimentada por una muestra aleatoria controlada del dataset unificado de 45 millones de filas. Tras realizar la partición de datos (70% para el entrenamiento de la IA y 30% para la validación de la prueba), el algoritmo arrojó las siguientes métricas de rendimiento y precisión reales:
* **Coeficiente de Determinación ($R^2$ en test):** **0.8277**, lo que significa que el modelo es capaz de explicar científicamente el 82.77% de la variabilidad del precio de las tarifas de NeoCab basándose en el entorno analizado.
* **Error Cuadrático Medio (RMSE en test):** **$6.58**, estableciendo el margen de desviación estándar promedio por predicción e indicando una alta precisión en la estimación de las tarifas operativas.

**Análisis de Coeficientes e Impacto de Variables:**
El código implementado extrajo exitosamente los coeficientes individuales de peso de cada variable de entrada (Celda 98). El análisis determinó que **la ubicación en hotspots de vida nocturna (`es_hotspot`) es la variable predictora con mayor peso operativo en el negocio, con un coeficiente de +$2.81**, seguida por la distancia del viaje (`distancia`) con un coeficiente de **+$1.75**, la precipitación con **+$0.27**, y la condición de día feriado (`es_feriado`) con un coeficiente negativo de **-$1.09**, lo que indica que estructuralmente en días feriados la tarifa base tiende a contraerse si no se aplican correctores dinámicos.
...
## 5.5. Dashboard Ejecutivo Interactiva de Control de Mando

> 💡 **Nota para la evaluación:** El siguiente panel es un entorno web dinámico integrado. Si está leyendo este documento en un visor con soporte HTML (como VS Code, un navegador web o un servidor local), puede desplazar el cursor sobre las barras y los puntos de dispersión para auditar las métricas exactas en tiempo real.

<iframe src="./online_viewer_net (1).html" 
        width="100%" 
        height="850px" 
        style="border: 2px solid #1a2a45; border-radius: 12px; background: #080c14; box-shadow: 0 12px 40px rgba(0,0,0,0.65);"
        loading="lazy">
</iframe>

*Nota: En caso de que su visor de Markdown bloquee los scripts interactivos locales por políticas de seguridad, puede ejecutar directamente el archivo independiente `online_viewer_net (1).html` adjunto en la carpeta de entrega.*
...
* **KPIs Asociados:**
  * *Tasa de Aceptación de Viajes con Surge:* Meta $\ge$ 88% (Monitoreo en tiempo real).
  * *Incremento en Ingreso por Viaje (RPT):* Meta +12% frente al baseline operativo.
  * *Elasticidad Precio-Demanda:* Meta $\le$ 5% de caída de volumen por cada 10% de recargo.
  * *Satisfacción del Cliente (NPS) / Churn Rate:* Meta NPS $\ge$ 65 y deserción mensual $\le$ 2%.
...
* **Insight de Negocio:** La redistribución física e institucional forzada de la flota no es financieramente viable debido a tres factores estructurales: el alto costo de oportunidad de retirar oferta de las zonas core de Manhattan, una captura marginal que no compensa los traslados vacíos y una tarifa promedio periférica inferior ($22.31). La verdadera oportunidad estratégica no radica en mover los autos de forma centralizada, sino en implementar un **modelo predictivo de incentivos algorítmicos opt-in** que motive la dispersión voluntaria y orgánica solo cuando el ratio de demanda local lo justifique.
